...
def generate_data(n_sample=10000, width=64):
    ...
    for i in range(n_sample):
        _x = np.random.rand(4)
        _im = draw_rect(_x, width)
        im.append(_im)
        # x.append(_x)
        x0, y0, x1, y1 = _x
        x.append(one_hot(norm(x0, width), norm(y0, width), width))
    x = np.stack(x)
    im = np.stack(im)  # (N, W, H)
    im = im.transpose(0, 3, 1, 2)  # (N, H, W, C) -> (N, C, H, W)

    indices = np.arange(0, n_sample, dtype="int32")
...
if __name__ == "__main__":
    ...
    # prepare dataset
    # generate_data(n_sample=10000, width=3)
    ...
    train_dataloader = utils.DataLoader(train_dataset, batch_size=1, shuffle=True)
    ...
    model = OnehotNet(width=3).to(device)
