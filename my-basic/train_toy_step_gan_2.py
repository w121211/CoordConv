...
# -------------------------------
# Dataset:
#   Real: n rectangles randomly placed, rectangle colors are
#   Fake: n rectangles randomly placed
#
# Toy experiment:
...
luminance = lambda r, g, b: (0.299 * r + 0.587 * g + 0.114 * b) / 255
# p = random.choice(PALATTES)
# p = sorted(
#     [(x, luminance(*ImageColor.getrgb(x))) for x in p], key=lambda x: x[1]
# )
...
class Generator(nn.Module):
    ...
    def forward(self, features, x_img):
        ...
        for i in range(self.n_masks):
            layer = rgb[:, i, :, :, :] * torch.ones(N, 1, self.imsize, self.imsize).to(self.device)
            mask = x_img[:, i:i+1, :, :]
...
class GANTrainer(object):
    ...
    def train(self):
        ...
        for epoch in range(opt.n_epochs):
            for i, (real_img, x_img) in enumerate(dataloader):
                ...
                features = self.Enc(x_img)
                fake_img = self.G(features, x_img)
                ...
                if i % opt.n_critic == 0:
                    ...
                    if batches_done % opt.sample_interval == 0:
                        ...
                        save_image(
                            denormalize(
                                fake_img, [0.5, 0.5, 0.5], [0.5, 0.5, 0.5]
                            ).data[:25],
                            os.path.join(
                        ...
                        save_image(
                            denormalize(
                                x_img[:, -3:, :, :], [0.5, 0.5, 0.5], [0.5, 0.5, 0.5]).data[:25],
                            os.path.join(
                                self.opt.sample_path,
                                "{:06d}_x.png".format(batches_done),
                            ),
                            nrow=5,
                            # normalize=True,
                        )
                        save_image(
                            denormalize(
                                real_img, [0.5, 0.5, 0.5], [0.5, 0.5, 0.5]
                            ).data[:25],
                            os.path.join(
                                self.opt.sample_path,
